...
class EllipticCurve_generic(WithEqualityById, plane_curve.ProjectivePlaneCurve):
    ...
    def __init__(self, K, ainvs):
        ...
        if self.discriminant() == 0:
            raise ArithmeticError(
                "invariants " + str(ainvs) + " define a singular curve")
    ...
    def is_x_coord(self, x):
        ...
        try:
            ...
            raise TypeError(
                'x must be coercible into the base ring of the curve')
...
#############################################################
    ...
    def multiplication_by_m_isogeny(self, m):
        ...
        phi._EllipticCurveIsogeny__initialize_rational_maps(
            precomputed_maps=(mx, my))
    ...
    def short_weierstrass_model(self, complete_cube=True):
        ...
        if K.characteristic() == 2:
            raise ValueError("short_weierstrass_model(): no short model for %s (characteristic is %s)" % (
    ...
    def plot(self, xmin=None, xmax=None, components='both', **args):
        ...
        try:
            ...
            raise NotImplementedError(
                "Plotting of curves over %s not implemented yet" % K)
        ...
        def f1(z): return (-(a1*z + a3) + sqrt(abs(d(z))))/2
        # Internal function for plotting second branch of the curve

        def f2(z): return (-(a1*z + a3) - sqrt(abs(d(z))))/2
        ...
        if xmin is None or xmax is None:
            xmins = []
            xmaxs = []
            if components in ['both', 'bounded'] and len(r) & gt
            1:
                xmins.append(r[0])
                xmaxs.append(r[1])
            ...
            # generated xmax.
            if components == 'unbounded' or components == 'both' and (len(r) == 1 or r[2] - r[1] & gt
                                                                      3*(r[1] - r[0])):
                flex = sorted(self.division_polynomial(
                    3).roots(RR, multiplicities=False))
                flex = flex[-1]
                xmins.append(r[-1])
                ...
                if a1 == 0:
                    ...
                    Ederiv = y**2 + a1*x*y + a3*y - \
                        (x**3 + a2*x**2 + a4*x + a6)
                critx = [a for a in Ederiv.roots(RR, multiplicities=False) if r[0] & lt
                         a & lt
                         r[1]]
                if len(critx) == 0:
                    raise RuntimeError(
                ...
                top_branch = ymax**2 + a1*x*ymax + a3 * \
                    ymax - (x**3 + a2*x**2 + a4*x + a6)
                bottom_branch = ymin**2 + a1*x*ymin + \
                    a3*ymin - (x**3 + a2*x**2 + a4*x + a6)
                xmaxs.append(max(top_branch.roots(
                    RR, multiplicities=False) + bottom_branch.roots(RR, multiplicities=False)))
            xmins = min(xmins)
            xmaxs = max(xmaxs)
            span = xmaxs - xmins
            if xmin is None:
        ...
        I = []
        if components in ['unbounded', 'both'] and xmax & gt
        r[-1]:
            ...
            = r[-1]:
                ...
                I.append((xmin, xmax, '='))
        if components in ['bounded', 'both'] and len(r) & gt
        1 and (xmin & lt
               r[1] or xmax & gt
                r[0]):
            if xmin & lt
        ...
        for j in range(len(I)):
            ...
            v = generate_plot_points(
                f1, (a, b), plot_points, adaptive_tolerance, adaptive_recursion, randomize)
            w = generate_plot_points(
                f2, (a, b), plot_points, adaptive_tolerance, adaptive_recursion, randomize)
            if shape == 'o':
