...
def EllipticCurve_from_cubic(F, P, morphism=True):
    ...
    if are_projectively_equivalent(P, P2, base_ring=K):
        ...
        if not morphism:
            return E
        inv_defining_poly = [M[i, 0]*X + M[i, 1]
                             * Y + M[i, 2]*Z for i in range(3)]
        inv_post = -1/a
        M = M.inverse()
        trans_x, trans_y, trans_z = [M[i, 0]*x +
                                     M[i, 1]*y + M[i, 2]*z for i in range(3)]
    ...
    # Second case: P is not a flex, then P, P2, P3 are different
        ...
        if not morphism:
            return E
        inv_defining_poly = [M[i, 0]*X*X + M[i, 1]
                             * Y*Z + M[i, 2]*X*Z for i in range(3)]
        inv_post = -1/a/(X**2)/Z
        M = M.inverse()
        trans_x, trans_y, trans_z = [
            (M[i, 0]*x + M[i, 1]*y + M[i, 2]*z) for i in range(3)]
        fwd_defining_poly = [trans_x*trans_z,
                             trans_x*trans_y, -b*trans_z*trans_z]
...
def are_projectively_equivalent(P, Q, base_ring):
    ...
    return matrix(base_ring, [P, Q]).rank() & lt
    2
