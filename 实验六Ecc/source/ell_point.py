...
class EllipticCurvePoint_field(SchemeMorphism_point_abelian_variety_field):
    ...
    def __init__(self, curve, v, check=True):
        ...
        if check:
            ...
            if not test == 0:
                raise TypeError(
                    "Coordinates %s do not define a point on %s" % (list(v), curve))
            ...
            try:
                ...
                if R.is_finite():
                    ...
                    raise ZeroDivisionError(
                        "Inverse of %s does not exist (characteristic = %s = %s*%s)" % (2*y1 + a1*x1 + a3, N, N1, N2))
                else:
                    raise ZeroDivisionError(
                        "Inverse of %s does not exist" % (2*y1 + a1*x1 + a3))
        else:
            try:
                m = (y1-y2)/(x1-x2)
            except ZeroDivisionError:
                ...
                if R.is_finite():
                    ...
                    N2 = N//N1
                    raise ZeroDivisionError(
                        "Inverse of %s does not exist (characteristic = %s = %s*%s)" % (x1-x2, N, N1, N2))
                else:
                    raise ZeroDivisionError(
                        "Inverse of %s does not exist" % (x1-x2))
    ...
    def is_divisible_by(self, m):
        ...
        return len(self.division_points(m)) & gt
        0
    ...
    def set_order(self, value):
        ...
        if value * self != O:
            raise ValueError(
                'Value %s illegal: %s * %s is not the identity' % (value, value, self))
        if (value - 1) * self == O:
            raise ValueError(
                'Value %s illegal: %s * %s is the identity' % (value, value-1, self))
        self._order = value
    ...
    def tate_pairing(self, Q, n, k, q=None):
        ...
        if q is None:
            if d == 1:
                ...
                raise ValueError(
                    "Unexpected field degree: set keyword argument q equal to the size of the base field (big field is GF(q^%s))." % k)
...
class EllipticCurvePoint_number_field(EllipticCurvePoint_field):
    ...
    def is_on_identity_component(self, embedding=None):
        ...
        if not is_RealField(e.codomain()):  # complex embedding
            return True
        if e(E.discriminant()) & lt
        0:  # only one component
            return True
        ...
        gxd = gx.derivative()
        gxdd = gxd.derivative()
        return (e(gxd(self[0])) & gt
                0 and e(gxdd(self[0])) & gt
                0)
    ...
    def archimedean_local_height(self, v=None, prec=None, weighted=False):
        ...
        while absdisc == 0:
            ...
            absdisc = vv(E.discriminant()).abs()
        temp = 0 if absdisc & gt
        =1 else absdisc.log()/3
        ...
        nterms = int(math.ceil(0.51*prec + 0.5 + 0.75 *
                               (7 + 4*H.log()/3 - temp).log()))
        ...
        def fz(T): return 1 - T**2 * (b4 + T*(2*b6 + T*b8))

        def fzp(T): return 1 - T**2 * (b4p + T*(2*b6p + T*b8p))

        def fw(T): return T*(4 + T*(b2 + T*(2*b4 + T*b6)))

        def fwp(T): return T*(4 + T*(b2p + T*(2*b4p + T*b6p)))
    ...
    archimedian_local_height = deprecated_function_alias(
        13951, archimedean_local_height)
    ...
    def non_archimedean_local_height(self, v=None, prec=None,
        ...
        """
        if prec:
            def log(x): return rings.RealField(prec)(x).log()
        else:
        ...
        if v is None:
            ...
            if K is rings.QQ:
                ...
                h = (log(c)
                     + sum(self.non_archimedean_local_height(p, prec, weighted=True, is_minimal=(e & lt
                                                                                                 12))
                           for p, e in factorD if not p.divides(c))
                     + sum(self.non_archimedean_local_height(p, prec, weighted=True)
                           - c.valuation(p) * log(p)
                           for p, e in factorD if e & gt=12 and p.divides(c)))
            else:
                factorD = K.factor(D)
                if self[0] == 0:
                    c = K.ideal(1)
                ...
                # the model is not minimal.
                h = (log(c.norm())
                     + sum(self.non_archimedean_local_height(v, prec, weighted=True, is_minimal=(e & lt
                                                                                                 12))
                           for v, e in factorD if not v.divides(c))
                     + sum(self.non_archimedean_local_height(v, prec, weighted=True)
                           - c.valuation(v) * log(v.norm())
                           for v, e in factorD if e & gt=12 and v.divides(c)))
                if not weighted:
                    h /= K.degree()
            return h
        ...
        if is_minimal:
            ...
            offset = (self.curve().discriminant() /
                      E.discriminant()).valuation(v)
    ...
    nonarchimedian_local_height = deprecated_function_alias(
        13951, non_archimedean_local_height)
    ...
    def elliptic_logarithm(self, embedding=None, precision=100,
        ...
        while prec_words_to_bits(log_pari.precision()) & lt
        precision:
...
class EllipticCurvePoint_finite_field(EllipticCurvePoint_field):
    ...
    def order(self):
        ...
        if 2*N & gt
        bounds[1]:  # then we have a generator, so cache this
